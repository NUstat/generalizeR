...
If the user needs to make adjustments to their inference population or if they have forgotten to store their results -- that is, to use the assignment operator `<-` to assign their results to an object (here called 'output') -- they may enter '2' to stop the function and do so. We have already taken care of both those items, however, so we enter '1' to proceed.

The next prompt requires the user to enter the name of the ID variable they wish to use. We enter the name of the column that contains the unique NCES school IDs in the CCD database -- `ncessch`:
